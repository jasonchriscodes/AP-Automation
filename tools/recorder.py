...
def OnMouseEvent(event):
    ...
    if event.Window:
        wrapped = app.connect(handle = event.Window)
    else:
        wrapped = None
    ...
    if "up" in event.MessageName: #and "down" in last_message:
        print ( "Handle:" + str(event.Window))    
        print ( "Message Name:" + event.MessageName)    
        print ( "Window Name:" + event.WindowName)   
        wrapped.top_window().print_control_identifiers()
        #print ( "Class :"+wrapped.Class())
        #print ( "Window Text:"+wrapped.WindowText())
        #print ('time.sleep(' +str((event.Time - last_event_time))+')')
        #print ('app%s.Window_(title = "%s", class_name = "%s").%sClickInput()'%( toplevel, wrapped.WindowText(), wrapped.Class(),  button))
...
def OnKeyboardEvent(event):
    ...
    if "down" in event.MessageName:
        ...
        print ('app.Window_(title = "%s", class_name = "%s").Typekeys("%s")'%(wrapped.window_text(), wrapped.class_name(), 'event.Key'))
