...
def updateDocStatus(docnumber,status):
    ...
    if(dbU.connect(dbhost,dbname,dbuser,dbpass)):
        dbU.update("update tblDocument set DocStatusID='"+str(status)+"' where DocField4='"+str(docnumber)+"'")
        dbU.close()
        l.add("Updated Ferret for PO" + str(docnumber) + " to status" + str(status))
...
logFileName = logLoc + "/" + "Log_"+str('%02d' % dt.day)+"_"+str('%02d' % dt.month)+"_"+str(dt.year)+ ".log"
l = Log(logFileName)
sync()
...
n= Notification("gmail",notifyTo,copyTo,senderEmail,senderEmailPass)
...
if(db.connect(dbhost,dbname,dbuser,dbpass)):
    ...
    db.execute("select * from PO_Ferret where Status='"+statusToSearch+"' AND FORMAT(CONVERT(DATE,INVOICE_DATE,103),'yyyyMM') <= '"+ period+"' AND doctotal IS NOT NULL ORDER BY PO_Number")
    ...
    db.setColumns({"Supplier":0,"PO_Number":1,"Status":2,"Ordered_By":3,"BranchCode":4,"Site":5,"Delivery_Addresss":6,"Warehouse":7,"Authorized_By":8,"UserConfirmed":9,"DateCreated":10,"POType":11,"PERIOD":12,"Invoice_No":13,"DocTotal":14,"invoice_date":15})
    ...
    for ind in range (0,rowCnt):
        try:
            ...
            if(detailsTotal == documentTotal and  (documentTotal!="$0.00") ):
                ...
                n.totals_did_not_match_notify(po_num,detailsTotal,documentTotal)
            ...
            if(match==False):
                ...
                cnt = 1
                if(cnt==1):
                    l.add("PO#:" + str(po_num) +  " successfully updated status for docid:"+str(docNo)+" to 16 n tblDocument table.")
                    n.set_Success_Fail(1,0)
                else:
                    l.add("PO#:" + str(po_num) +  " status updation for docid:"+str(docNo)+" in tblDocument failed.")
