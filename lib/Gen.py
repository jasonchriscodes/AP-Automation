...
class Obj:
    ...
    def __init__(self, win_name, par=None, l=None):
        try:
            ...
            if (l):
                self.l.add('Problem occured finding window:' +
                           win_name + ', error: ' + str(e))
            raise Exception('Problem occured finding window:' +
                            win_name + ', error: ' + str(e))
    ...
    def print_child_ele_details(self, spaces, root):
        ...
        for ele in eles:
            ...
            if (rTxt and not (rTxt in self.objs)):
                out = spaces + "["
                out = out + "control_type:" + \
                    str(ele.element_info.control_type)+","
                out = out + "name:" + str(ele.element_info.name)+","
                out = out + "class_name:" + \
                    str(ele.element_info.class_name)+","
                out = out + "control_id:" + \
                    str(ele.element_info.control_id)+","
